...
@Entity
@Table(name = "meals", uniqueConstraints = {@UniqueConstraint(columnNames = {"user_id", "date_time"}, name = "uk_meals_user_datetime")})
@NamedQueries({
        ...
        @NamedQuery(name = Meal.UPDATE, query = "UPDATE Meal m SET m.description=:description, m.dateTime=:dateTime, m.calories=:calories WHERE m.id=:id AND m.user.id=:userId")
})
